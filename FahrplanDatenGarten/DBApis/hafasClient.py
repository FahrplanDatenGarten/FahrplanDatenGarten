...
class HafasClient():
    ...
    key = bytes([97, 72, 54, 70, 56, 122, 82, 117,
                 105, 66, 110, 109, 51, 51, 102, 85])

    def searchLocation(self, term):
        data = {"svcReqL": [{"meth": "LocMatch", "req": {
            "input": {"field": "S", "loc": {"name": term, "type": "S"}}}}]}
                ...
                if response_part["err"] == "OK" and response_part["meth"] == "LocMatch":
                    for result in response_part["res"]["match"]["locL"]:
                        search_results.append(
                            {k: v for k, v in result.items() if k in ["lid", "name", "type"]})
    ...
    def parse_timedelta(self, time_str):
        regex = re.compile(
            r'(?P<days>\d{2})(?P<hours>\d{2})(?P<minutes>\d{2})(?P<seconds>\d{2})')
        if len(time_str) == 6:
    ...
    def sendPostRequest(self, data, headers={}):
        # from res/raw/haf_config.properties of DBNavigator
        data["auth"] = {"aid": self.aid2, "type": "AID"}
        data["client"] = {
            "id": "DB",
            "name": "DB Navigator",
            "type": "IPH",
    ...
    def strpDelta(self, string):
        return datetime.timedelta(hours=int(
            string[-6:-4]), minutes=int(string[-4:-2]), seconds=int(string[-2:]))
